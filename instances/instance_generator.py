...
def gen_z_values(n_tasks):
	z = []
	for i in range(n_tasks):
		z.append(random.randint(0,num_islands-1))
...
def gen_tasks(inst_lines):
	...
	tasks.insert(3, 'z', gen_z_values(len(tasks['task_no'])))
	# print(tasks)
...
def choose_point(all_positions, mn, mx):
	point = (random.randint(mn, mx), random.randint(mn, mx))
	while point in all_positions:
		point = (random.randint(mx, mx), random.randint(mn, mx))
...
def gen_machines(tasks):
	...
	machines = [[0, 0, num_islands-1, pt[0], pt[1], spd]]
	for i in range(1,n_machines):
		lz = random.randint(0, num_islands-2)
		hz = random.randint(lz+1, num_islands-1)
		pt = choose_point(all_positions, mn, mx)
