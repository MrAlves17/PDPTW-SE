...
function meloFormulation(inst::InstanceData, params::ParameterData)
	...
	for k in inst.K
		for i in inst.V[2:length(inst.V)]
			for j in inst.V[2:length(inst.V)]
				if (i,j) in inst.A
					@constraint(model, x[i,j,k] + x[j,i,k] <= 1, base_name = "cNovo")
				end
			end
		end
	end
	...
	sol = createSolutionMelo(inst,x,z)
...
function createSolutionMelo(inst::InstanceData, x, z)
	M = 999999
	...
	for k in inst.K
		...
		while i != length(inst.Vprime)
			...
			for j in inst.Vprime
				...
				# 	i = j
				# end
				if i != j && (i,j) in inst.A && x[i,j,k] > 0
					i = j
					break
				end
			end
